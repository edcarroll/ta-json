...
function deserializeRootObject(object:JsonValue, type:Function = Object):any {
    ...
    if (childDefinitions.length > 0) {
        ...
        const [childType] = childDefinitions.find(([type, def]) => def.discriminatorValue == values[parentDefinition.discriminatorField]);

        if (childType) {
            return deserializeRootObject(object, childType);
        }
    }
